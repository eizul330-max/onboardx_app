...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  try {
    final team = await _teamService.getTeamByNoTeam(_teamController.text.trim());
    if (team != null) {
      setState(() {
        _isValidTeam = true;
        _selectedTeam = team;
  ...
  if (!_isValidTeam || _selectedTeam == null) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Please enter a valid team number'), backgroundColor: Colors.red),
    );
    return;
  }
  ...
  try {
    ...
    if (user != null) {
      // build profile payload - send team_no (and optional team_id)
      final profileData = {
        'full_name': _nameController.text.trim(),
        'username': _usernameController.text.trim(),
        'phone_number': _phoneController.text.trim(),
        'work_type': _selectedWorkType,
        'team_no': int.tryParse(_teamController.text.trim()),
        // optionally include Mongo team _id if available from previous API
        'team_id': _selectedTeam != null ? _selectedTeam!['_id'] ?? _selectedTeam!['id'] : null,
      };

      final ok = await _firebaseAuth.syncUserProfile(user, profileData);

      // Send email verification
      await user.sendEmailVerification();

      // Sign out to force verify flow
      ...
      if (ok) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Registration saved locally but failed to sync to server.'), backgroundColor: Colors.orange),
        );
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
